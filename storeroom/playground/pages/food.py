...
from database import add_food, delete_food, get_food, update_food
...
with st.form("add_food_form"):
    ...
    category = shadcn.Select(
        label="Food Category", options=["snack", "light", "moderate", "full", "heavy"]
    )
    submit = st.form_submit_button("Add Food")

if submit:
    try:
        food_time = datetime.strptime(time_str, "%Y-%m-%d %H:%M").replace(
            tzinfo=timezone.utc
        )
        if food_time > now:
    ...
    with col3:
        if st.button("Edit", key=f"edit_food_{food['id']}"):
            ...
            st.experimental_set_query_params(
                page="food", action="edit", food_id=food["id"]
            )
            st.experimental_rerun()
...
if query_params.get("action") == ["edit"] and "edit_food" in st.session_state:
    ...
    with st.form("edit_food_form"):
        ...
        category = shadcn.Select(
            label="Food Category",
            options=["snack", "light", "moderate", "full", "heavy"],
            value=food["category"],
        )
