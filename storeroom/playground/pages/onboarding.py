...
with st.form("onboarding_form"):
    ...
    height = shadcn.NumberInput(
        label="Height (cm)", min_value=100, max_value=250, value=170
    )
    weight = shadcn.NumberInput(
        label="Weight (kg)", min_value=30, max_value=200, value=70
    )
    sex = shadcn.Select(label="Sex", options=["M", "F"])
    driver_profile = shadcn.Select(
        label="Driver Profile", options=["general", "novice", "professional"]
    )
...
if submitted:
    ...
    update_user_details(
        user["id"], dob_str, float(height), float(weight), sex, driver_profile
    )
