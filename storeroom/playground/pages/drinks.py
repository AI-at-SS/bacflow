...
from database import add_drink, delete_drink, get_drinks, update_drink
...
with st.form("add_drink_form"):
    ...
    vol = shadcn.NumberInput(
        label="Volume (liters)", min_value=0.0, max_value=2.0, step=0.01, value=0.33
    )
    alc_prop = shadcn.NumberInput(
        label="Alcohol Proportion (0-1)",
        min_value=0.0,
        max_value=1.0,
        step=0.01,
        value=0.05,
    )
    time_str = shadcn.TextInput(
        label="Time of Consumption (YYYY-MM-DD HH:MM)",
        placeholder=now.strftime("%Y-%m-%d %H:%M"),
    )
    sip_interval = shadcn.NumberInput(
        label="Sip Interval (minutes)", min_value=1, max_value=60, step=1, value=1
    )
    submit = st.form_submit_button("Add Drink")

if submit:
    try:
        drink_time = datetime.strptime(time_str, "%Y-%m-%d %H:%M").replace(
            tzinfo=timezone.utc
        )
        if drink_time > now:
...
for drink in drinks:
    ...
    with col1:
        st.write(
            f"{drink['name']} - {drink['vol']} L, {drink['alc_prop'] * 100}% at {drink['time']}"
        )
    with col2:
    ...
    with col3:
        if st.button("Edit", key=f"edit_{drink['id']}"):
            ...
            st.experimental_set_query_params(
                page="drinks", action="edit", drink_id=drink["id"]
            )
            st.experimental_rerun()
...
if query_params.get("action") == ["edit"] and "edit_drink" in st.session_state:
    ...
    with st.form("edit_drink_form"):
        ...
        alc_prop = shadcn.NumberInput(
            label="Alcohol Proportion (0-1)",
            value=drink["alc_prop"],
            min_value=0.0,
            max_value=1.0,
            step=0.01,
        )
        time_str = shadcn.TextInput(
            label="Time (YYYY-MM-DD HH:MM)", value=drink["time"]
        )
        sip_interval = shadcn.NumberInput(
            label="Sip Interval",
            value=drink["sip_interval"],
            min_value=1,
            max_value=60,
            step=1,
        )
    ...
    if updated:
        try:
            new_time = datetime.strptime(time_str, "%Y-%m-%d %H:%M").replace(
                tzinfo=timezone.utc
            )
            if new_time > now:
