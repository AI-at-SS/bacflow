...
def generate_coherent_demographic_features(num_entries):
    ...
    for i in range(num_entries):
        base_weight = (
            50 + (heights[i] - 150) * 0.5
        )  # Basic weight increases linearly with height.
        if genders[i] == "male":
...
def generate_synthetic_trajectory_with_devices(
    participant_key: str,
    trajectory_key: int,
    ...
    acc_data_smartphone = np.random.normal(
        loc=0, scale=sensor_variance, size=(num_samples, 3)
    )
    gyro_data_smartphone = np.random.normal(
        loc=0, scale=sensor_variance / 2, size=(num_samples, 3)
    )
    acc_data_smartwatch = np.random.normal(
        loc=0, scale=sensor_variance * 1.5, size=(num_samples, 3)
    )
    gyro_data_smartwatch = np.random.normal(
        loc=0, scale=sensor_variance, size=(num_samples, 3)
    )
    ...
    timestamps = [
        start_time + timedelta(seconds=i / frequency) for i in range(num_samples)
    ]
...
if __name__ == "__main__":
    metadata_df, timeseries_df = generate_synthetic_trajectory_with_devices(
        participant_key=1,
        trajectory_key=1,
        duration_seconds=60,
        label="[0.00]",
