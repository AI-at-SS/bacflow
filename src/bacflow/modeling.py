...
def calculate_body_factor(person: Person, model: str) -> float:
    ...
    return (
        _M_calculate_body_factor(person, model)
        if person.sex == Sex.M
        else _F_calculate_body_factor(person, model)
    )


def calc_aer(sex: Sex, bac: float) -> float:
    """Alcohol elimination rate (AER) based on Simic & Tasic (2007)."""
    aer = (0.16 if sex == Sex.F else 0.14) + (bac * 0.05)
...
def calculate_bac_for_model(
    ...
    for i in range(1, len(model_bac_ts)):
        ...
        model_bac_ts.at[i, "eliminated"] = model_bac_ts.at[i - 1, "eliminated"] + min(
