...
from bacflow.schemas import Drink, FoodIntake, Model, Person, Sex
...
def sample_food():
    ...
    return FoodIntake(time=dt, category="heavy")
...
def test_cumulative_absorption():
    ...
    values = absorption_df.loc[idx_after_drink, "kg_absorbed"].to_numpy()
...
def test_simulate():
    ...
    person = Person(DoB=date(1995, 1, 31), height=1.75, weight=70, sex=Sex.M)
    ...
    for col in ["time", "bac", "bac_perc"]:
        assert col in sim_results[Model.Seidl].columns
...
def test_aggregate_simulation_results():
    ...
    times = pd.date_range(start=BASE_TIME, periods=10, freq="min")
...
def test_identify_threshold_times():
    ...
    times = pd.date_range(start=BASE_TIME, periods=20, freq="min")
